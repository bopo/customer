...
var Players = {
    ...
    Show: function() {
        ...
        $('#topcc').html('<div id="playppvod" style="height:26px;line-height:26px;overflow:hidden">正在播放：' + list_name + ' ' + vod_name + ' ' + this.UrlName + '</div>');
        $('#topleft').html('<a href="' + this.LastWebPage + '">上一集</a> <a href="' + NextWebPage + '">下一集');
        $('#topright').html('<a onClick="Players.ShowList();" style="cursor:pointer">开启/关闭列表</a>');

        if(this.PlayerName=='bdhd')
            $('#playleft').html('<div id="BdInstall"></div><div id="BdPlayer"></div>');
        else
            $('#playleft').html('<iframe src="' + this.Buffer + '" id="buffer" name="buffer" width="100%" height="' + this.Height + '" scrolling="no" frameborder="0" style="display:none;position:absolute;z-index:9;"></iframe>' + $Showhtml());

        $('#playright').height(this.Height + 'px');
        $('#playright').css('display', list_show);
        $('#playright').html('<div id="list" style="height:' + this.Height + 'px">' + this.CreateList() + '</div>');
    },
    BufferHide: function() {
        $("#buffer").hide();
    },
    CreateList: function() {
        ...
        for (var i = 0; i < count_play; i++) {
            ...
            for (var j = 0; j < count_urls.length; j++) {
                var href = this.ParentUrl.replace('video/' + this.Sid + '/' + this.Pid, 'video/' + i + '/' + (j + 1)).replace(this.Id + '/' + this.Sid + '/' + this.Pid, this.Id + '/' + i + '/' + (j + 1));
                li_on    = (this.Sid == i && this.Pid == (j + 1)) ? ' class="list_on"' : '';
    ...
    Tabs: function(no, n) {
        ...
        $('#main' + no).addClass('h2_on');
        $('#sub' + no).toggle();
    },
    Install: function() {
        var downurl = eval('ff_' + this.PlayerName);

        if(this.PlayerName == 'xigua')
            $('#install').html('<iframe border="0" src="' + downurl + '" marginWidth="0" frameSpacing="0" marginHeight="0" frameBorder="0" noResize scrolling="no" width="100%" height="' + this.Height + '" vspale="0"></iframe>').show();
        else
            $('#install').html('<iframe border="0" src="/player/install.html?playname=' + this.PlayerName + '&u=' + downurl + '" marginWidth="0" frameSpacing="0" marginHeight="0" frameBorder="0" noResize scrolling="no" width="100%" height="' + this.Height + '" vspale="0"></iframe>').show();
    },
    Html: function() {
        $("#player").html('<div class="FF"><table border="0" cellpadding="0" cellspacing="0"><tr><td colspan="2"><table><tr><td width="100" id="topleft"></td><td id="topcc"></td><td width="100" id="topright"></td></tr></table></td></tr><tr><td colspan="2" id="install" style="display:none"></td></tr><tr><td id="playleft" valign="top"> </td><td id="playright" valign="top"> </td></tr></table></div>');
    },
    Play: function() {
        this.Html();
        var URL  = this.ParentUrl.match(/\d+.*/g)[0].match(/\d+/g);
        this.Id  = URL[(URL.length - 3)] * 1;
        this.Sid = URL[(URL.length - 2)] * 1;
        this.Pid = URL[(URL.length - 1)] * 1;

        if(this.Sid >= this.ParentPlayerName.length)
            this.Sid = this.ParentPlayerName.length - 1;
        ...
        if(vod_urlname != '')
            this.UrlName = vod_urlname;
        else
            this.UrlName = UrlArr[this.Pid - 1].split("++")[0];

        var LastPid      = Math.max(Math.abs(this.Pid - 1), 1);
        this.LastWebPage = this.ParentUrl.replace('video/' + this.Sid + '/' + this.Pid, 'video/' + this.Sid + '/' + LastPid).replace(this.Id + '/' + this.Sid + '/' + this.Pid, this.Id + '/' + this.Sid + '/' + LastPid);

        var NextPid      = Math.min(this.Pid + 1, UrlArr.length);
        this.NextWebPage = this.ParentUrl.replace('video/' + this.Sid + '/' + this.Pid, 'video/' + this.Sid + '/' + NextPid).replace(this.Id + '/' + this.Sid + '/' + this.Pid, this.Id + '/' + this.Sid + '/' + NextPid);

        if (this.Url == this.NextUrl)
            this.NextWebPage = '';

        if(this.PlayerName == 'bdhd')
            $("#player").after('<link type="text/css" href="http://player.baidu.com/lib/setupax/load.css" rel="stylesheet">');

        $("#player").after('<scr'+'ipt type="text/javascript" src="/assets/scripts/player/' + this.PlayerName + '.js?v=20130828"></scr'+'ipt>');

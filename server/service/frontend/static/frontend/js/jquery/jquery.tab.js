	...
	$.fn.tab=function(options){
		...
		tag.css({"cursor":"pointer"}).each(function(i){
			tag.eq(i).bind(org.act,function(){
				...
				k=i;
				switch(org.effact){
					case "slow": con.eq(i).show("slow",function(){org.fun(con.eq(i));}).siblings(org.conTag).hide("slow");
					break;
					case "fast":con.eq(i).show("fast",function(){org.fun(con.eq(i));}).siblings(org.conTag).hide("fast");
					break;
					case"scrollx":conwrp.animate({left:-i*conwh+"px"},org.aniSpeed);
					break;
					case "scrolly" : conwrp.animate({top:-i*conht+taght+"px"},org.aniSpeed);
					break;
					default:con.eq(i).show(0,function(){org.fun(con.eq(i));}).siblings(org.conTag).hide();
					break;
				}
			})
		});

...
class StockStaticFilesHandler(StaticFilesHandler):
    def _should_handle(self, path):
        return path.startswith((settings.STATIC_URL, settings.MEDIA_URL))

    def get_response(self, request):
        response = super(StockStaticFilesHandler, self).get_response(request)
...
def banner():
    ...
    # the connection Stock, with some exceptions where we
    ...
    db_version_func = {
        ...
        # The remaining Stocks haven't actually been tested,
        # and so their version logic has been gleaned from glancing
        # at the code for each Stock.
...
class Command(runserver.Command):
    ...
    def get_handler(self, *args, **options):
        ...
        if settings.DEBUG or options["insecure_serving"]:
            handler = StockStaticFilesHandler(handler)
